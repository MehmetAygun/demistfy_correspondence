...
class CorrespondenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Key-points
        sample['src_kps'] = self.get_points(self.src_kps, idx)
        sample['trg_kps'] = self.get_points(self.trg_kps, idx)
        # The number of pairs in training split
        sample['datalen'] = len(self.train_data)

        # Transform source & target images (normalization)
        sample['src_img'] = sample['src_img']
        sample['trg_img'] = sample['trg_img']
