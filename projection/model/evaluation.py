...
class Evaluator:
    ...
    def __init__(self, benchmark, device):
        ...
        self.eval_buf = {
            ...
            'spair':    {'pck': [], 'cls_pck': dict(), 'pck_swap':[], 'pck_miss':[], 'pck_jitter':[]},
            'caltech':  {'ltacc': [], 'iou': []}
        }
    ...
    def eval_pck(self, prd_kps, data):
        ...
        jitter, miss, swap = correct_kps_analyze(data['trg_kps'].cuda(), prd_kps, pckthres, data['alpha'])
...
def correct_kps_analyze(trg_kps, prd_kps, pckthres, alpha=0.1):
    ...
    swap = (nn_gt != closest_kps) * torch.le( l2dist, thres * alpha*2)
    swap = torch.sum(swap)
    
    return jitter, miss, swap 
