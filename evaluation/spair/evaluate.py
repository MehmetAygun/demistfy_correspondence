...
def run(datapath, featurepath, benchmark, thres, alpha, logpath, layers, w, model_type, model, transform, dataloader=None, visualize=False, usebbox=False):
    ...
    for idx, data in enumerate(dataloader):
        ...
        src_img_path = os.path.join(args.datapath,'SPair-71k', 'JPEGImages', data['pair_class'][0],data['src_imname'][0])
        trg_img_path = os.path.join(args.datapath,'SPair-71k', 'JPEGImages', data['pair_class'][0],data['trg_imname'][0])
        ...
        if usebbox: 
            bbox = data['trg_bbox']*data['trg_intratio']
        else:
            bbox = None
        
        # c) Predict key-points & evaluate performance
        prd_kps = pred_kps(data['src_img'], data['trg_img'], src_feature, trg_feature, data['src_kps'], w = w, layers=layers, bbox=bbox)
        prd_kps = prd_kps.to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--featurepath', type=str, default='/data1/SPair-71k/features_resnet50/')
    parser.add_argument('--dataset', type=str, default='spair')
    parser.add_argument('--thres', type=str, default='auto', choices=['auto', 'img', 'bbox'])
    parser.add_argument('--alpha', type=float, default=0.1)
    parser.add_argument('--logpath', type=str, default='')
    parser.add_argument('--visualize', action='store_true')
    parser.add_argument('--usebbox', action='store_true')
    parser.add_argument('--layers', type=str, default='1,4')
    parser.add_argument('--projpath', type=str, default='') 
    parser.add_argument('--dimredpath', type=str, default='') 
    parser.add_argument('--cnnsize', type=int, default=384) 
    ...
    if model_type == "resnet50": #resnet50
        ...
        net_w, net_h = args.cnnsize, args.cnnsize
        resize_mode="minimal"
    ...
    if not args.finetuned_model == '':
        model = torch.load(args.finetuned_model)
    if model_type != 'vit' and model_type != 'dino':
        model.eval()
    
    run(datapath=args.datapath, featurepath=args.featurepath, benchmark=args.dataset, 
        thres=args.thres, alpha=args.alpha, logpath=args.logpath, 
        visualize=args.visualize, layers=args.layers, usebbox=args.usebbox, 
